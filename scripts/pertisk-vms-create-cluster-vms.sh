#!/usr/bin/env bash
# Create N control-plane + M worker VMs on pertisk-vms from a Pertisk cloud qcow2,
# then by default continue into pertisk-vms-lab-up (DHCP IPs → bootstrap → join → CNI).
#
# Auth: PERTISK_VMS_URL, PERTISK_VMS_USER, PERTISK_VMS_PASSWORD,
#       PERTISK_VMS_STORAGE, PERTISK_VMS_NETWORK.
#
# Examples:
#   ./scripts/pertisk-vms-create-cluster-vms.sh --cp-vmid 210 --workers 2
#   ./scripts/pertisk-vms-create-cluster-vms.sh --cp-vmid 210 --controlplanes 1 --workers 0 --no-lab-up
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
UPLOAD="${ROOT}/scripts/pertisk-vms-upload-vm.sh"
LAB_UP_SH="${ROOT}/scripts/pertisk-vms-lab-up.sh"
...
CP_MEMORY="${PERTISK_VMS_CP_MEMORY:-}"
CP_CORES="${PERTISK_VMS_CP_CORES:-}"
WORKER_MEMORY="${PERTISK_VMS_WORKER_MEMORY:-}"
WORKER_CORES="${PERTISK_VMS_WORKER_CORES:-}"
DISK_GB="${PERTISK_VMS_DISK_GB:-}"
CP_DISK_GB="${PERTISK_VMS_CP_DISK_GB:-}"
WORKER_DISK_GB="${PERTISK_VMS_WORKER_DISK_GB:-}"
CP_VMID="${CP_VMID:-210}"
CONTROLPLANES="${CONTROLPLANES:-1}"
WORKERS="${WORKERS:-2}"
NAME_PREFIX="${NAME_PREFIX:-pertisk}"
DISK="${PERTISK_VMS_DISK:-${PROXMOX_DISK:-${ROOT}/out/pertisk-cloud-amd64.qcow2}}"
CP_DISK="${PERTISK_VMS_CP_DISK:-}"
WORKER_DISK="${PERTISK_VMS_WORKER_DISK:-}"
DO_LAB_UP=1
LAB_UP_EXTRA=()
...
if [[ -z "${PERTISK_VMS_URL:-}" ]]; then
  echo "PERTISK_VMS_URL unset. Export PERTISK_VMS_URL / PERTISK_VMS_USER / PERTISK_VMS_PASSWORD." >&2
  exit 1
fi
...
if [[ -n "${PERTISK_VMS_STATIC_IPS:-}" ]]; then
  read -ra STATIC_IPS_ARRAY <<<"${PERTISK_VMS_STATIC_IPS}"
fi
STATIC_IP_IDX=0
...
for i in $(seq 1 "$CONTROLPLANES"); do
  ...
  if [[ $STATIC_IP_IDX -lt ${#STATIC_IPS_ARRAY[@]} ]]; then
    ...
    [[ -n "${PERTISK_VMS_STATIC_GATEWAY:-}" ]] && UPLOAD_ARGS+=(--gateway "${PERTISK_VMS_STATIC_GATEWAY}")
...
Stopped after VM create (--no-lab-up). Continue with:
  ./scripts/pertisk-vms-lab-up.sh --skip-build --skip-vms --cp-vmid ${CP_VMID} \\
    --controlplanes ${CONTROLPLANES} --workers ${WORKERS} --prefix ${NAME_PREFIX}
EOF
  exit 0
fi

echo "==> continuing → pertisk-vms-lab-up (IPs → cluster → CNI)"
