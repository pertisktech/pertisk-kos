...
`controlplanes` and `workers` are **create-time only**. Scale later with [`pertisk-kos_node`](node.md). Changing VM sizing (`cp_*` / `worker_*`), network, or CNI forces **replace**.
...
* `provider_id` - (String) Existing mgmt provider UUID (`pertisk-kos_provider` id). Forces new resource.

### Optional

* `controlplanes` - (Number) Initial control-plane count at create (default `1`). Create-time only; later HCL drift is ignored. Use `pertisk-kos_node` to scale. Forces new when changed before create.
* `workers` - (Number) Initial worker count at create (default `1`). Create-time only. Scale with `pertisk-kos_node`.
...
```shell
terraform import pertisk-kos_cluster.lab 5826cf0e-9c33-4f15-b71f-252ca1110443
```
