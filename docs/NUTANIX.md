...
Create imports the hashed qcow2 **once**, then clones VMs in parallel (`PERTISK_VM_JOBS`, default 4). Mgmt runs one exclusive job **per cluster** (create/upgrade/delete); different clusters run in parallel. Add-on installs also run in parallel.
