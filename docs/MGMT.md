...
When a cluster is **ready**, **Add-ons** uses group tabs (**Certificates**, **Ingress**, **Storage & network**). It checks live install state and applies optional cluster apps via `kubectl` (and Helm for Pertisk Ingress) on the management host. CoreDNS and metrics-server stay bootstrap basics (not this tab).
...
Mgmt runs **one exclusive cluster job at a time** (create, delete, upgrade, add/remove/resize node). **Add-on installs** skip that global queue and run in parallel. Delete cancels that cluster’s queued work, **aborts** a running create/upgrade/add-on, and removes job rows when the cluster is gone — so a new create is not stuck `queued` behind a leftover delete.
