...
| `MGMT_IMAGE_REGISTRY` | Container registry host for Ingress / Dashboard / scaler / CD images (`image.registry` / `image.repository` in Helm values). Default `registry.tools.thaidevops.co` (Docker Registry V2 / common registry). |
| `MGMT_IMAGE_REGISTRY_USER` / `MGMT_IMAGE_REGISTRY_PASSWORD` | Optional. Only for **private** registries. Leave unset when the registry allows anonymous pull. Used by image add-ons when the form leaves Registry user/password empty. |
...
| Pertisk Ingress | image tag (default `0.1.95`); optional admin host; TLS secret list (or HTTP only); optional admin password; optional registry user/password (leave empty for public registry) | Helm `pertisk-ingress` from `--repo` (`MGMT_HELM_CHART_REPO`, default `https://charts.tools.thaidevops.co`) into `pertisk-proxy`, image from `MGMT_IMAGE_REGISTRY` (default `registry.tools.thaidevops.co`) |
