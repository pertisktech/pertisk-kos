...
| 4.2.11 | Rotate kubelet server cert | partial | `serverTLSBootstrap: true`; lab-up and pertisk-mgmt approve serving CSRs from **registered** nodes (create, add/adopt, pertisk-kos-scaler); production still wants a dedicated validating CSR approver |
...
6. Core Kubernetes deliberately leaves kubelet serving CSRs Pending. Lab-up and pertisk-mgmt approve `kubernetes.io/kubelet-serving` requests only from registered `system:node:` identities (including workers added by pertisk-kos-scaler). Production still needs a dedicated validating CSR approver for rotation policy.
